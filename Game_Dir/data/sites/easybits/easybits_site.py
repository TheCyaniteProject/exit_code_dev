...
def web_site(web_page, go_from):
        ...
        if 'easybits.com' in settings.login_creds[settings.username.lower()]:
            if settings.login_creds[settings.username.lower()]['easybits.com'][0] == True:
                if user.lower() == settings.login_creds[settings.username.lower()]['easybits.com'][1].lower():
                    ...
                    print('\n%s\nUser %s logged into easybits.com @ %s' % (
                        settings.get_time(), settings.username, easybits_account))
                    easybits_logOn()
                    return
    ...
    Label(web_page, text='easyBits.com - Free Anonamous Banking.', bg='gold', font=(None, 10)).pack(anchor=W, side=LEFT,
                                                                                                    fill=Y,
                                                                                                    expand=False)
    ...
    def easybits_main():
        ...
        easybits_pass.pack()
        Button(easybits_login, text=' Login ', bg='gold', cursor='hand2', command=easybits_doLogin).pack(padx=6, pady=5,
                                                                                                         anchor=S,
                                                                                                         side=RIGHT)
        ...
        if not sitelocker == None:
            ...
            sitelocker_label = Label(easybits_page, text='SiteLocker v%s Secured' % str(sitelocker), fg='blue',
                                     cursor='hand2')
    ...
    easybits_logout_button = Button(web_page, text=' Logout ', bg='gold', relief=FLAT, cursor='hand2',
                                    command=easybits_doLogout)
    ...
    def easybits_logOn():
        ...
        Label(account_frame, text='Account: %s' % (easybits_account), fg='orange', font=(None, 15)).pack(padx=20,
                                                                                                         pady=10,
                                                                                                         anchor=NW)
        amount_label = Label(account_frame, text='Balance: %sB' % (wallet.check('easybits', easybits_account)),
                             fg='orange', font=(None, 15))
        ...
        def sell_easybits():
            ...
            def authorize():
                reload(wallet)  # Unresolved References in 3.6 Anaconda
                global token
