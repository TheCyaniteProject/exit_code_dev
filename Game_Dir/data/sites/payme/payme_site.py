...
# reload(wallet)  # Anaconda 4.3.1 Python 3.6 incompatible?
"""========================================================="""
...
def web_site(web_page, go_from):
    ...
    def payme_doLogin():
        ...
        if 'payme.net' in settings.login_creds[settings.username.lower()]:
            if settings.login_creds[settings.username.lower()]['payme.net'][0] == True:
                if user.lower() == settings.login_creds[settings.username.lower()]['payme.net'][1].lower():
                    ...
                    print('\n%s\nUser %s logged into payme.net @ %s' % (
                        settings.get_time(), settings.username, payme_account))
                    payme_logOn()
                    return
        ...
        if flag == 'to_notExist':
            ...
            if flag == False:
                print(
                    '\n%s\nUser %s was denied access to payme.net @ %s\nReason: Your IP is blacklisted for this account.' % (
                        settings.get_time(), settings.username, payme_account))
                return
            elif flag == None:
            ...
            raise Exception('unexpectedReturnValue')

    payme_page = LabelFrame(web_page)
    payme_page.pack(anchor=W, side=BOTTOM, fill=BOTH, expand=True)
    Label(web_page, text='Payme.net - Get Paid.', bg='green', font=(None, 10)).pack(anchor=W, side=LEFT, fill=Y,
                                                                                    expand=False)
    ...
    def add_payme_autofill():
        ...
        for usern in make_users:
            new_autofill = Label(payme_autofill, text=('[%s]' % (make_users[usern])), fg='blue', cursor='hand2',
                                 bg='white')
    ...
    def payme_main():
        ...
        payme_pass.pack()
        Button(payme_login, text=' Login ', bg='white', cursor='hand2', command=payme_doLogin).pack(padx=6, pady=5,
                                                                                                    anchor=S,
                                                                                                    side=RIGHT)
        ...
        if not sitelocker == None:
            ...
            sitelocker_label = Label(payme_page, text='SiteLocker v%s Secured' % str(sitelocker), fg='blue',
                                     cursor='hand2')
    ...
    payme_logout_button = Button(web_page, text=' Logout ', bg='green', relief=FLAT, cursor='hand2',
                                 command=payme_doLogout)
    ...
    def payme_logOn():
        ...
        Label(account_frame, text='Account: %s' % (payme_account), fg='darkgreen', font=(None, 15)).pack(padx=20,
                                                                                                         pady=10,
                                                                                                         anchor=NW)
        amount_label = Label(account_frame, text='Balance: $%s' % (wallet.check('payme', payme_account)),
                             fg='darkgreen', font=(None, 15))
