...
from data.settings import login_creds, get_time, user_dir, username
...
def check_email():
    retval = {}
    for file in os.listdir('%s%s\\sites\\email\\mail\\' % (user_dir, username)):
        if file.endswith('.log'):
            log = {}
            chat = []
            with open('%s%s\\sites\\email\\mail\\%s' % (user_dir, username, file), 'r') as f:
                f = f.read()
            ...
            for line in chat_log.split('\n'):
                if '<msg=' in line:
                    chat.append([chat_log2.split('<msg=', 1)[1].split('>', 1)[0].strip().rstrip(),
                                 chat_log2.split('>', 1)[1].split('</msg>', 1)[0].strip().rstrip()])
...
def web_site(web_page, go_from):
    ...
    sitelocker = None
    riddleme = None
    with open('%s%s\\sites\\email\\site.ini' % (user_dir, username), 'r') as site_settings:
    ...
    def email_doLogin():
        ...
        if user.strip().rstrip() == '':
            return
        if user.lower().strip().rstrip() in login_creds:
            for i in login_creds[user.lower().strip().rstrip()]:
                if i == 'email.com':
                    if login_creds[user.lower().strip().rstrip()][i][0] == True:
                        email_account = login_creds[user.lower().strip().rstrip()][i][1].strip().rstrip()
                        print('\n%s\nUser %s logged into Email.com @ %s' % (
                            get_time(), username, email_account))
                        email_logOn()
        else:
            with open('%s%s\\sites\\email\\logins.ini' % (user_dir, username), 'r') as logins:
    ...
    def add_email_autofill():
        ...
        for usern in login_creds:
            for site in login_creds[usern]:
                if site == 'email.com':
                    if login_creds[usern][site][0] == True:
                        autofill_TF = True
                        make_users[usern] = login_creds[usern][site][1]
    ...
    Label(web_page, text='Email.com - Say it with Email!', bg='lightblue', font=(None, 10)).pack(anchor=W, side=LEFT,
                                                                                                 fill=Y, expand=False)
    ...
    def email_main():
        ...
        if not sitelocker == None:
            ...
            sitelocker_label = Label(email_page, text='SiteLocker v%s Secured' % str(sitelocker), bg='white', fg='blue',
                                     cursor='hand2')
    ...
    email_logout_button = Button(web_page, text=' Logout ', bg='lightblue', relief=FLAT, cursor='hand2',
                                 command=email_doLogout)
                    ...
                    def call():
                        ...
                        for msg in convo:
                            if '<link' in msg[1]:
                                ...
                                links.append([links_var.split('text=', 1)[0].strip().rstrip(),
                                              links_var.split('text=', 1)[1].split('>', 1)[0].strip().rstrip()])
                                ...
                                while '<link' in links_var:
                                    ...
                                    link = [links_var.split('<link=', 1)[1].split('text=', 1)[0].strip().rstrip(),
                                            links_var.split('text=', 1)[1].split('>', 1)[0].strip().rstrip()]
                                    if len(link[1]) > 40:
                                        cut_link(link[1])
                                    else:
                                    ...
                                    links_var = links_var.split('>', 1)[1]
                            if '\n' in msg[1]:
                                # global lines removed because you've already declared in line 305
                                # http://stackoverflow.com/questions/2105611/name-times-is-used-prior-to-global-declaration-but-it-is-declared
                                lines = []
                            ...
                            if msg[0] == 't':
                                ...
                                Label(label, text=msg[1], bg='lightblue', justify=LEFT).pack(anchor=W,
                                                                                             side=TOP)  # FROM Person
                                for var in links:
                                    def make_link():
                                        link = var[0]
                                        hotlink = Label(label, text=var[1], bg='lightblue', fg='blue', cursor='hand2',
                                                        justify=LEFT)
                                        hotlink.pack(anchor=W, side=TOP)
...
def hack_in(user=False):
    ...
    if user.lower().strip().rstrip() in login_creds:
        for i in login_creds[user.lower().strip().rstrip()]:
            if i == 'email.com':
                email_account = login_creds[user.lower().strip().rstrip()][i][1]
                print('\n%s\nUser %s hacked into Email.com @ %s' % (
                    get_time(), username, email_account))
                email_logOn()
    else:
        with open('%s%s\\sites\\email\\logins.ini' % (user_dir, username), 'r') as logins:
