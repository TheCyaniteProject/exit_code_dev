...
# Anaconda 4.3.1 Python 3.6 incompatible? reload(apps)
# Anaconda 4.3.1 Python 3.6 incompatible? reload(wallet)
"""========================================================="""
...
def check_flag(flag, name, buy_frame, buy_framem):
    if flag == 'fileAlreadyExists':
        ...
        Label(buy_frame, text='You already own this file\n(or one with the same name)', bg='black', fg='purple').pack(
            anchor=N, side=TOP)
        ...
        Label(buy_frame, text=name, bg='black', fg='purple').pack(anchor=N, side=TOP)
        Label(buy_frame,
              text='SYSTEM ERROR: fileNotFound\nPlease contact the developer.\nhttp://twitter.com/exit_code_dev',
              bg='black', fg='purple').pack(anchor=N, side=TOP)
        Button(buy_frame, text='Done', bg='purple', fg='black', command=buy_framem.destroy).pack(anchor=S, side=RIGHT)
        return
    else:
        buy_frame.destroy()
        ...
        Label(buy_frame, text=name, bg='black', fg='purple').pack(anchor=N, side=TOP)
        Label(buy_frame,
              text='SYSTEM ERROR: unknownError\nPlease contact the developer.\nhttp://twitter.com/exit_code_dev',
              bg='black', fg='purple').pack(anchor=N, side=TOP)
        Button(buy_frame, text='Done', bg='purple', fg='black', command=buy_framem.destroy).pack(anchor=S, side=RIGHT)
        return
...
def web_site(web_page):
    ...
    Label(page_frame, text='{DataKult}', bg='black', fg='purple', font=('Courier', 16)).pack(anchor=W, side=LEFT,
                                                                                             fill=Y, expand=False)
    ...
    Label(main_page, text='Name | Discription | $/B', bg='black', fg='purple', font=('Courier', 10)).pack(anchor=S,
                                                                                                          side=LEFT,
                                                                                                          fill=X,
                                                                                                          expand=False)
    ...
    for file in os.listdir('%s..\\sites\\datakult\\software\\' % (settings.user_dir)):
        ...
        def make_listing(file):
            ...
            def buyeb_click(name, price, file):
                def do_buy(buy_frame, name, buy_framem):
                    if wallet.check('easybits', settings.easybits_ac, price) == False:
                        ...
                        Button(buy_frame, text='Done', bg='purple', fg='black', command=buy_framem.destroy).pack(
                            anchor=S, side=RIGHT)
                    else:
                    ...
                    if flag:
                        ...
                        check_flag(flag, name, buy_frame, buy_framem)
                ...
                buy_frame.pack(pady=50)
                Label(buy_frame, text=name, bg='black', fg='purple').pack(anchor=N, side=TOP)
                Label(buy_frame, text='Are you sure you want to buy\n%s for %sB?' % (name, price), bg='black',
                      fg='purple').pack(anchor=N, side=TOP)
                Button(buy_frame, text='Buy', bg='purple', fg='black',
                       command=lambda: do_buy(buy_frame, name, buy_framem)).pack(anchor=S, side=LEFT)
                Button(buy_frame, text='Cancel', bg='purple', fg='black', command=buy_framem.destroy).pack(anchor=S,
                                                                                                           side=RIGHT)
            ...
            def buyfree_click(name, file):
                ...
                buy_frame.pack(pady=50)
                Label(buy_frame, text=name, bg='black', fg='purple').pack(anchor=N, side=TOP)
                Label(buy_frame, text='Are you sure you want to download\n%s?' % (name), bg='black', fg='purple').pack(
                    anchor=N, side=TOP)
                Button(buy_frame, text='Download', bg='purple', fg='black',
                       command=lambda: do_buy(buy_frame, name, buy_framem)).pack(anchor=S, side=LEFT)
                Button(buy_frame, text='Cancel', bg='purple', fg='black', command=buy_framem.destroy).pack(anchor=S,
                                                                                                           side=RIGHT)
            ...
            def more_click(name, desc, price):
                ...
                Button(buy_frame, text='Close', bg='purple', fg='black', command=buy_framem.destroy).pack(anchor=S,
                                                                                                          side=RIGHT)
            ...
            Label(product_frame, text='%s | %s |' % (file_name, file_summary), bg=background, fg=foreground,
                  font=('Courier', 10)).pack(side=LEFT, expand=False)
            Button(product_frame, text='Read More', fg=foreground, bg=background,
                   command=lambda: more_click(file_name, file_desc, price)).pack(padx=10, side=LEFT, expand=False)
            if buy_easybits == True:
                Button(product_frame, text='Buy for %sB' % file_easybits, fg=foreground, bg=background,
                       command=lambda: buyeb_click(file_name, file_easybits, file_file)).pack(padx=1, side=RIGHT,
                                                                                              expand=False)
            if buy_payme == True:
                Button(product_frame, text='Buy for $%s' % file_payme, fg=foreground, bg=background,
                       command=lambda: buypm_click(file_name, file_payme, file_file)).pack(padx=1, side=RIGHT,
                                                                                           expand=False)
            if free_downl == True:
                Button(product_frame, text='Free Download', fg=foreground, bg=background,
                       command=lambda: buyfree_click(file_name, file_file)).pack(padx=1, side=RIGHT, expand=False)
