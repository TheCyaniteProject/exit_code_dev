...
reload_scripts()
for i in player_scripts: print i
        ...
        if len(args) >= 1:
            for i in args:
                if i in '--help':
                    ...
                    try: arg = cmd_help.parse_args(args)
                    except: return
...
===========================================\n  """
            ...
            if 'do_'+args[0] in dir(Console):
                ...
                eval(fn)
            else: print 'help: error: help could not find "%s"' % args[0]
        else: print 'help: error: you can only pass one object at a time'
...
positional arguments:
  <command>   Use 'help <command>' for specific help."""
        if len(args) >= 1:
            for i in args:
                if i in '--help':
                    ...
                    # Processes the command - Capture '--help' or argument errors (e.g: Typos)
                    try: arg = cmd_dir.parse_args(args)
                    except: return
    ...
    def do_apps(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_apps.parse_args(args)
    ...
    def do_scripts(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_scripts.parse_args(args)
        except SystemExit: return
    ...
    def do_dc(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_disconnect.parse_args(args)
        except SystemExit: return
    ...
    def do_connect(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_connect.parse_args(args)
        except SystemExit: return

        # Main program code
        if arg.web:
            if tkinter_ui.disable_2V == True:
    ...
    def do_clear(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_clear.parse_args(args)
        except SystemExit: return
    ...
    def do_quick(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_quick.parse_args(args)
        except SystemExit: return
    ...
    def do_whois(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_whois.parse_args(args)
        except SystemExit: return
    ...
    def do_nmap(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_nmap.parse_args(args)
        except SystemExit: return
    ...
    def do_cd(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_cd.parse_args(args)
        except SystemExit: return
    ...
    def do_cat(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_cat.parse_args(args)
        except SystemExit: return
    ...
    def do_ls(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_ls.parse_args(args)
        except SystemExit: return
    ...
    def do_del(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_del.parse_args(args)
        except SystemExit: return
    ...
    def hdo_cheat(self, args):
        ...
        # Processes the command - Capture '--help' or argument errors (e.g: Typos)
        try: arg = cmd_cheat.parse_args(args)
        except SystemExit: return

        cmdcheat(arg.cheat)

command_list = []
shown_c = 0 ; hidden_c = 0
...
def cmdcheat(cheat):
    if 'exploit_addpayme' in cheat:
        ...
        try: settings.player_rank = settings.player_rank + int(cheat)
        except: print 'error: problem'
    elif 'exploit_setrank' in cheat:
        cheat = cheat.split('exploit_setrank')[1].strip().rstrip()
        try: settings.player_rank = int(cheat)
        except: print 'error: problem'
        ...
        try:
            ...
            if settings.player_exp <= 0: settings.player_exp = 0
        except: print 'error: problem'
...
def cmdcat(file):
    if not get_ufile_path(file) == False:
    ...
    if not file.endswith(settings.text_types):
        ...
        return
    if not can_run == True:
        if not settings.player_home in file_dir:
...
def cmddel(file):
    if file.strip().rstrip() == '*':
        ...
        for file in os.listdir(filedir):
            ...
            except: print 'del: error: "%s" could not be removed' % file
        reload_autofill()
    ...
    if os.path.isfile(file_dir):
        os.remove(file_dir)
        print 'del: "%s" was removed' % file
    else: print 'del: error: "%s" is not a valid file' % file
    reload_autofill()
    tkinter_ui.concom.set_completion_list(command_list)
...
def cmddir(dir, printd=True):
    ...
    if not location == False:
        if not location.endswith('\\'):
            location = location+'\\'
    else:
        printd = False
    if printd == True:
        if not settings.player_home in location:
            if can_run == True:
...
def run(command):
    ...
    with stdout_redirect(StringIO.StringIO()) as new_stdout:
        ...
        if not command.strip() == '':
            ...
            try: sys_args = shlex.split(command)
        ...
        if not dobreak == True:
            if 'do_'+sys_args[0] in dir(Console):
                ...
                if script.endswith('.app'):
                    if not get_ufile_path(script) == False:
                    ...
                    print_out = True
                    running_script = False
                else: print '"%s" is not a valid command, app, or script' % script
