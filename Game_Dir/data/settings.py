...
pronouns   =    {
                    ...
                    'male': ['He', 'His', 'His'],
                    'floofball': ['"RainbowDashies\' Pyromancing Floof-ball"',
...
def load_game_settings():
    ...
    if not os.path.isfile(settings_file) == True:
...
def load_player_settings():
    ...
    global email_ac
    global player_ip
    if not os.path.isfile(('%s%s\\user.ini' % (user_dir,username.lower())).split(game_dir+'\\')[1]) == True:
...
if not payme_ac == None:
    with open('%s%s\\wallets\\payme\\%s.ini' % (user_dir,username.lower(),payme_ac), 'r') as f:
        ...
        except: print '!!!Could not load Payme Balence!!!'

if not easybits_ac == None:
    with open('%s%s\\wallets\\easybits\\%s.ini' % (user_dir,username.lower(),easybits_ac), 'r') as f:
        ...
        except: print '!!!Could not load Easybits Balence!!!'

# User Email Loading
if not email_ac == None:
    try:login_creds[username.lower()]['email.com'] = [True, str(email_ac.strip().rstrip())]
    except: print '!!!Could not load Email account!!!'
    ...
    if os.path.isfile('%s%s\\systems\\discoveredips.ini' % (user_dir,username.lower())):
        with open('%s%s\\systems\\discoveredips.ini' % (user_dir,username.lower()), 'r') as f:
            for system in f.readlines():
                ...
                else: print 'orphaned system: %s from DISCOVERED' % system
load_discovered_IPs()
    ...
    if os.path.isfile('%s%s\\systems\\visitedips.ini' % (user_dir,username.lower())):
        with open('%s%s\\systems\\visitedips.ini' % (user_dir,username.lower()), 'r') as f:
            for system in f.readlines():
                ...
                else: print 'orphaned system: %s from VISITED' % system
load_visited_IPs()
...
print 'game_seed = %s' % game_seed
print '=====\n'

print '=====\nAccount Ranking: %s | Rank: %s - EXP: %s/%s\n=====\n' % (username,player_rank,player_exp,exp_multi)
print '=====\nAccount Balance: %s | Payme: %s - Bits: %s\n=====\n' % (username,payme,bits)
...
print 'Moved player @ %s' % working_dir
