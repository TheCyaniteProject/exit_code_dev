...
class TempPage(object):
    ...
    def link_payme(self, link):  # payme.net/auth=?-acc=xxxxxxxxxxxxxxxxxx-ip=xxx.xxx.xxx.xxx
        ...
        email = '''
            ...
            The Payme Team.
            <link:https://www.payme.net, text:[www.Payme.net]>
            ''' % (email_address, link[2], link[1])
...
def sitelocker(disable=False):
    global sitelocker_ver
    if (disable is False) or (disable == 0):
        return sitelocker_ver
    elif (disable is True) or (disable == 1):
        if sitelocker_ver is False:
