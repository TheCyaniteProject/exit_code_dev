...
def save_confirm(file, save_frame=False, save_as=False):
    ...
    def file_save(mode=True):
        if save_as is not False:
            save_as.destroy()
        if save_frame is not False:
            save_frame.destroy()
        if file_frame is not False:
            file_frame.destroy()
...
#no_warning
if settings.no_warning is False:
